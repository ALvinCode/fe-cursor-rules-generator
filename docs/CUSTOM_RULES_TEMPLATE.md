...
在生成基础规则的同时，系统会自动生成一个自定义规则模板文件（`custom-rules.mdc`），供用户填写项目特定的自定义规则。

## 核心特性

### 1. 自动生成
- 每次运行 `generate_cursor_rules` 时，会自动生成模板文件
- 文件位置：`.cursor/rules/custom-rules.mdc`
- 包含完整的元数据配置和编写指导
...
### 2. 查看模板
打开 `.cursor/rules/custom-rules.mdc`，查看完整的编写指导。
...
1. **文件命名**: 文件名固定为 `custom-rules.mdc`，不要修改
...
### Q: 文件会被重新生成覆盖吗？
A: 是的。每次运行 `generate_cursor_rules` 时，如果文件不存在或内容仍然是模板状态，会被重新生成。如果已经填写了内容，建议在生成前备份。
